...
router.get('/total/:birthDate', async(req, res, next) => {
    try {
        ...
        const total_number = (today_index * my_index * new Date().getDate()) % 180;
        ...
        const final_index = final_index_array[total_number];

        const result = await S087.findOne({
            where: { DB_express: final_index },
            attributes: ['DB_data']
        });
